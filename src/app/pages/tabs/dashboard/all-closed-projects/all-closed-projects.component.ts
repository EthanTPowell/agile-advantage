...
@Component({
  selector: 'app-all-closed-projects',
  templateUrl: './all-closed-projects.component.html',
  styleUrls: ['./all-closed-projects.component.scss'],
  providers: [SortByDatePipe, TimeAgoPipe, ProjectItemDataService]
})
export class AllClosedProjectsComponent implements OnInit {
    ...
    this.authenticationService.checkAuth().then((userAuth: any) => {
      if (userAuth) {
        this.userDataService.getOne(userAuth.uid).subscribe((user) => {
          ...
          this.projectItemDataService.getProjectClosed(this.user.projectId).pipe(tap((projectItems: ProjectItemDto[]) => {
